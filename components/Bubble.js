...
const Wrap = styled.div`
  ...
  .helpers .line {
    stroke: var(--color-helper-line);
  }

  .helpers .outline {
    stroke-width: 4;
    stroke: var(--color-helper-outline);
  }

  .helpers .radius {
    stroke-dasharray: 4;
    stroke: var(--color-helper-area);
  }

  .helpers .dot {
    fill: var(--color-helper-vertex);
    stroke-width: 0;
  }
...
const Bubble = ({
  ...
  const returnVector = useCallback(
    (start, end) => ({ x: end.x - start.x, y: end.y - start.y }),
    []
  )

  const returnDistance = useCallback(
    (start, end) =>
      Math.sqrt(Math.pow(end.x - start.x, 2) + Math.pow(end.y - start.y, 2)),
    []
  )

  const returnSegments = useCallback(
    (totalLength) => {
  ...
  const returnEndPoints = useCallback(
    (segments) =>
  ...
  const returnShapeSegments = useCallback(
    (points, center) => {
      if (!center) return []
